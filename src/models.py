...
Each function follows a consistent interface:
  ...
  - Returns a fitted estimator (and, where relevant, evaluation metrics)
  - Saves the model to outputs/models/ if save=True
"""
...
LATE_DELIVERY_CATEGORICALS: list[str] = []   # add e.g. "vehicle_type" if available


def train_late_delivery_model(
    df: pd.DataFrame,
    features: list[str] | None = None,
    target: str = "late_delivery_flag",
    test_size: float = 0.20,
    random_state: int = 42,
    save: bool = True,
) -> tuple[Pipeline, dict]:
    """
    Train a late-delivery risk classifier.

    Returns
    -------
    pipeline : fitted sklearn Pipeline
    metrics  : dict with precision, recall, f1, roc_auc
    """
    ...
    if save:
        path = MODELS_DIR / "late_delivery_model.joblib"
...
def train_demand_model(
    ...
    """
    Train a demand forecasting model (Random Forest regressor).
    ...
    if save:
        path = MODELS_DIR / "demand_forecast_model.joblib"
        joblib.dump(pipeline, path)
        print(f"Model saved: {path}")
...
def cluster_customers(
    ...
    """
    Cluster customers using KMeans on the selected features.

    Default features: order_frequency, avg_order_value, avg_distance_km

    Returns
    -------
    df_out  : original DataFrame with new column `customer_cluster`
    ...
    if not features:
        raise ValueError("No valid feature columns found in customer_df.")
    ...
    customer_df.loc[X.index, "customer_cluster"] = model.fit_predict(X_scaled)
    customer_df["customer_cluster"] = customer_df["customer_cluster"].astype("Int64")

    print("── Customer Clustering ──────────────────────────────")
    print(customer_df["customer_cluster"].value_counts().sort_index())

    if save:
        path = MODELS_DIR / "customer_cluster_model.joblib"
        joblib.dump({"model": model, "scaler": scaler, "features": features}, path)
        print(f"Model saved: {path}")
...
def cluster_routes(
    ...
    """
    Cluster routes using KMeans.

    Default features: total_distance, stop_count, vehicle_utilization,
    ...
    if not features:
        raise ValueError("No valid feature columns found in routes_df.")

    X = routes_df[features].dropna()
    scaler = StandardScaler()
    X_scaled = scaler.fit_transform(X)
    ...
    routes_df.loc[X.index, "route_cluster"] = model.fit_predict(X_scaled)
    routes_df["route_cluster"] = routes_df["route_cluster"].astype("Int64")

    print("── Route Clustering ─────────────────────────────────")
    print(routes_df["route_cluster"].value_counts().sort_index())

    if save:
        path = MODELS_DIR / "route_cluster_model.joblib"
        joblib.dump({"model": model, "scaler": scaler, "features": features}, path)
        print(f"Model saved: {path}")
